...
export function shouldExclude(relativePath, exclude) {
...
function isUnknownFileType(message) {
    return message.includes('Unknown file type');
}
export async function uploadQueuedFiles(options) {
    const { files, srcDir, dest, accountId, logger, assets, exclude = [], concurrency = DEFAULT_CONCURRENCY, attempts = DEFAULT_ATTEMPTS, timeout = DEFAULT_TIMEOUT_MS, } = options;
